...
sudo docker run -d --name cep-temperatura-test -p 8080:8080 -e WEATHER_API_KEY="$WEATHER_API_KEY" "$TAG"
...
sudo docker stop cep-temperatura-test
sudo docker rm cep-temperatura-test
